#!/usr/bin/env python3
"""
Generate Android device frame template PNG (Pixel-style, centered hole-punch camera).
...
BEZEL = 15
SCREEN_CORNER_R = 62
# Android hole-punch front camera (centered), instead of an iPhone Dynamic Island
HP_D = 34                # hole-punch diameter
HP_TOP = 22              # offset from top of screen
...
def generate():
    ...
    # ── Android hole-punch front camera (centered) ──────────────────
    hp_x = (DEVICE_W - HP_D) // 2
    hp_y = screen_y + HP_TOP
    ImageDraw.Draw(frame).ellipse(
        [hp_x, hp_y, hp_x + HP_D, hp_y + HP_D],
        fill=(0, 0, 0, 255),
    )
