...
2. Confirm you can read them: list the image files found and their pixel sizes. Flag any image whose
   width is below ~1080px (still usable, just lower-res).
3. Decide an **output folder** in the user's current project: default **`aso-output/`** (create
   `aso-output/scaffolds/` and `aso-output/prompts/`). Tell the user where output will go.
...
3. Build every scaffold with a SINGLE bash command (one permission prompt). For each screenshot:
...
python3 "$SKILL_DIR/compose.py" \
  ...
  --output "aso-output/scaffolds/01-track.png"
```

`--verb` = line 1, `--desc` = line 2 (both auto-uppercased & centered), `--bg` = background hex.
Output is a 1290×2796 PNG with the title + Android phone frame + the screenshot composited in.
...
For EACH scaffold, write `aso-output/prompts/<NN-slug>.md` containing a tailored prompt. Use this
template, filling the bracketed parts from what's actually visible on that screenshot:

```
The attached image is a DRAFT LAYOUT (scaffold) for a Google Play Store screenshot. Transform it into
a polished, photorealistic, high-converting Play Store marketing screenshot.
...
ENHANCE: Replace the flat placeholder phone with a photorealistic modern Android flagship phone
(Pixel-style: slim uniform bezels, a small centered hole-punch front camera, rounded screen corners) —
clearly an ANDROID phone, NOT an iPhone (no notch, no pill). Realistic material, subtle reflections,
soft drop shadow. Keep it in the same position and size as the draft. Keep the background a clean, flat,
solid colour — no gradients, glows or patterns. Make the headline crisp.
...
Output tall portrait 9:16, highest resolution. No watermarks, no extra text, no app-store chrome, no hands.
```
...
exactly, so this looks like part of the same set."

Also write `aso-output/prompts/README.md` indexing each scaffold ↔ its prompt file ↔ its title.

## Phase 6 — Hand off (STOP here)

Tell the user clearly:
1. Open **ChatGPT**, attach `aso-output/scaffolds/<NN-slug>.png`, paste the matching
   `aso-output/prompts/<NN-slug>.md`. Do #1 first; for the rest, also attach the finished #1.
2. (Optional) To make all finals a uniform 1080×1920, run the resize snippet in this skill's README.
3. Upload the finished images to Play Console in order. Google needs 2–8 phone screenshots.
